...
function efecto_nieve_menu(){
	...
	add_menu_page('Navidad IndianWebs', 'Navidad IndianWebs','administrator', 'navidad-indianwebs.php', 'efecto_nieve_action', plugins_url('indianwebslogo.png', __FILE__));
...
function mostrar_efecto_nieve(){
	...
	if($set_copos=='Yes'){
		...
		$script .= '<script>jQuery(document).snowfall({flakeCount : 25, image :"'.plugins_url('/images/snowflake.png', __FILE__).'", minSize: 10, maxSize:20, maxSpeed : 3});</script>';
...
add_action( 'wp_enqueue_scripts', 'mostrar_nieve' );
